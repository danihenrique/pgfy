...
async function Api(options) {
  try {
    ...
    self.setErrorHandler((error, request, reply) => {
      console.log('setErrorHandler->Error: ', error);
      console.log('setErrorHandler->Method: ', request.req.method);
      console.log('setErrorHandler->Request', request.req.originalUrl);
      reply.send(error);
    });
    self.start = start;
